...
Exact caching is enabled by default with an in-memory development store. Cache keys use SHA-256 and isolate tenant, model, prompt version, sampling parameters, roles, and message content. The response header `X-LingShu-Cache` and metadata field `cacheStatus` report `MISS` or `EXACT`. Tool-enabled conversations always report `MISS` because their cache processors are intentionally skipped.
